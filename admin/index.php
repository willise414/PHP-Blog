...
// fetch current users posts from DB - not all posts

if (isset($_SESSION['user_is_admin'])) {
  $admin_query = "SELECT id, title, body, category_id from POSTS";
  $posts = mysqli_query($connection, $admin_query);
} else {

  $current_user_id = $_SESSION['user-id'];
  $query = "SELECT id, title, body, category_id from POSTS WHERE author_id = $current_user_id ORDER BY id DESC";
...
// if ($current_user_id = $_SESSION['user-id']) {
//   $query = "SELECT id, title, body, category_id from POSTS WHERE author_id = $current_user_id ORDER BY id DESC";
//   $posts = mysqli_query($connection, $query);
// } elseif ($current_user_id = $_SESSION['user_is_admin']) {
//   $query = "SELECT id, title, body, category_id from POSTS ORDER BY id DESC";
//   $posts = mysqli_query($connection, $query);
// }
?>
    ...
    <aside>
      <ul>
        ...
        <?php if (isset($_SESSION['user_is_admin'])) :  ?>
          ...
          <li>
            <a href="add-category.php"><i class="uil uil-edit-alt"></i>
              <h5>Add Category</h5>
            </a>
          </li>
          <li>
        ...
        <table>
          ...
          <tbody>
            <?php while ($post = mysqli_fetch_assoc($posts)) : ?>
              <!-- get category name of each post from categorise table -->
